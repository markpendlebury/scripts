...
vault.login.aws.base() {
    if [[ -z $VAULT_TOKEN ]]; then
        ...
        OUTPUT=$(vault read aws/creds/"$VAULT_ROLE" -format=json) && aws configure set aws_access_key_id "$(echo "$OUTPUT" | jq -r .data.access_key)" --profile "$VAULT_PROFILE" && aws configure set aws_secret_access_key "$(echo "$OUTPUT" | jq -r .data.secret_key)" --profile "$VAULT_PROFILE"
    fi
}
